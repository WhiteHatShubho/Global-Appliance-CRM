...
import '../utils/deadline-loading.css';

const GlobalLoaderSVG = () => {
  useEffect(() => {
    // Set animation durations for deadline loader
    ...
    if (progressFill && deathGroup) {
      progressFill.style.animationDuration = `${animationTime}s`;
      deathGroup.style.animationDuration = `${animationTime}s`;
    }
    
    if (designerArm) {
      // Progressively speed up designer arm animation
      let currentSpeed = 1.5;
      const speedUp = setInterval(() => {
        if (currentSpeed > 0.2) {
          currentSpeed -= 0.3;
          if (designerArm) {
            designerArm.style.animationDuration = `${Math.max(0.2, currentSpeed)}s`;
          }
        }
      }, 4000);
...
export default GlobalLoaderSVG;
